...
const CommentsSection = () => {
  ...
  const fetchComments = async (page = 1, searchTerm = searchKeyword, options = {}) => {
    ...
    try {
      ...
      if (!isSuccessResponse(res)) {
        if (showToast) toast.error(res?.message || "댓글 목록 조회에 실패했습니다.");
        return;
      }
  ...
  useEffect(() => {
    fetchComments(1, "", { showToast: false });
  ...
  const onSearch = () => {
    ...
    fetchComments(1, term, { showToast: true, isSearching: true });
  };
  ...
  const onPageChange = (page) => fetchComments(page, searchKeyword);

  const onView = (row) => {
    setSelectedCommentNo(row.commentNo);
    setIsDetailOpen(true);
  };

  const onDelete = async (row) => {
    const isDeleted = row.commentStatus === "Y";
    ...
    try {
      const response = await axiosAuth.deleteWithBody(
        `/api/admin/community/comments/${row.commentNo}`,
        {
      ...
      if (!response || response === "" || isSuccessResponse(response)) {
        ...
        if (message && message.includes("성공")) {
          ...
          toast.error(message || `댓글 ${action}에 실패했습니다.`);
        }
      }
    } catch (error) {
  ...
  const handleDiaryClick = (diaryNo) => {
    if (diaryNo) {
  ...
  const columns = [
    { key: "commentNo", label: "번호", thStyle: { width: 90, textAlign: "center" } },
    ...
    { key: "memberName", label: "작성자", thStyle: { width: 120 } },
    {
      ...
      render: (row) => {
        ...
        return (
          <button
            type="button"
            onClick={() => handleDiaryClick(row.diaryNo)}
            style={{
              background: "none",
              border: "none",
              color: "#2563eb",
  ...
  const actions = [
    ...
    {
      key: "delete",
      label: (row) => row?.commentStatus === "N" ? "삭제" : "복구",
      variant: (row) => row?.commentStatus === "N" ? "danger" : "primary",
  ...
  return (
    <>
      ...
      <DataTable
        ...
        actions={actions}
        emptyTitle={loading ? "데이터를 불러오는 중..." : "댓글 데이터가 없습니다"}
        emptyDesc={loading ? "" : "일기에 댓글이 달리면 여기서 관리할 수 있어요."}
      />
