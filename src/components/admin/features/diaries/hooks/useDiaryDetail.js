...
export const useDiaryDetail = (diaryNo, enabled = true) => {
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      try {
        setLoading(true);
        setError(null);
        const res = await axiosAuth.getList(`/api/admin/community/diaries/${diaryNo}`);
        
        if (!isSuccessResponse(res)) {
          const errorMsg = res?.message || "일기 상세 조회에 실패했습니다.";
          setError(errorMsg);
