...
const FileUpload = ({
  ...
  return (
    <>
      <DropZone $isDragging={isDragging} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}>
        <input ref={fileInputRef} type="file" accept={accept} onChange={handleFileInput} />
        <DropContent $isDragging={isDragging}>
          <UploadIcon $isDragging={isDragging}>{isDragging ? "📂" : "📁"}</UploadIcon>
          <UploadText>
            {isDragging ? (
              "파일을 여기에 놓으세요"
            ) : (
          ...
          <UploadHint>JPG, PNG 형식 (최대 {Math.round(maxSize / 1024 / 1024)}MB)</UploadHint>
        </DropContent>
      </DropZone>
